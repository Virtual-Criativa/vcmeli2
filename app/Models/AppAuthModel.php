...
class AppAuthModel
{
	...
	public function __construct()
	{
		...
		$this->collections = $this->mongodbConfig->collections;
	...
	public function clearForgottenPasswordCode(string $identity): bool
	{
		...
		if ($count > 0)
		{
			$this->db
				->where($this->identityColumn, $identity)
				->set(array('forgotten_password_code' => NULL, 'forgotten_password_time' => NULL, 'forgotten_password_selector' => null))
	...
	public function identityCheck(string $identity=''): bool
	{
		...
		$document = $this->db->where($this->identityColumn, $identity)
							->limit(1)
							->get($this->collections['users']);
	...
	public function register(string $identity, string $password, string $email, array $additionalData=[], array $groups=[])
	{
		...
		$query = $this->db->where(['name' => $this->config->defaultGroup], 1)->get($this->tables['groups']);
		if (! isset($query[0]["_id"]) && empty($groups))
		{
	...
	public function login(string $identity, string $password, bool $remember=false): bool
	{
		...
		$this->triggerEvents('extra_where');
		$document = $this->db
			->select(array($this->identityColumn, '_id', 'username', 'email', 'password', 'active', 'last_login', 'first_name', 'last_name'))
			...
			// stomach, goveg! :))
			->where($this->identityColumn, (string) $identity)
			->limit(1)
			->get($this->collections['users']);
		...
		if (count($document) === 1)
		{
			...
			if ($this->verifyPassword($password, $user->password, $identity))
			{
				...
				$this->updateLastLogin($user->{'_id'}->{'$id'});
	...
	public function getAttemptsNum(string $identity, $ipAddress=null): int
	{
		if ($this->config->trackLoginAttempts)
		{
			...
			$this->db->where(array('login' => $identity));
			...
			if ($this->config->trackLoginIpAddress)
			{
				...
				$this->db->where(array('ip_address' => $ipAddress));
				//$builder->where('ip_address', $ipAddress);
			}
			$this->db->where_gt('time', time() - $this->config->lockoutTime);
			$builder = $this->db->get('login_attempts');
	...
	public function increaseLoginAttempts(string $identity): bool
	{
		if ($this->config->trackLoginAttempts)
		{
			...
			$builder = $this->db->insert('login_attempts', $data);
	...
	public function clearLoginAttempts(string $identity, int $oldAttemptsAxpirePeriod=86400, $ipAddress = null): bool
	{
		...
		if ($this->config->trackLoginIpAddress)
		{
			...
			$dbResponse = $this->db
				...
				->where_or(array('time <' => time() - $oldAttemptsAxpirePeriod))
	...
	public function updateLastLogin(string $id): bool
	{
		...
		//$affectedRows = $this->db->update($this->tables['users'], ['last_login' => time()], ['_id' => $id]);
		...
		return $affectedRows->getMatchedCount();
	}
	...
	public function setSession(\stdClass $user): bool
	{
		...
		$sessionData = [
			...
			'email'               => $user->email,
			'user_id'             => $user->{'_id'}->{'$id'}, //everyone likes to overwrite id so we'll use user_id
